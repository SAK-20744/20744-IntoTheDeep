...
import org.firstinspires.ftc.teamcode.config.farBucketAuto;
...
//@Disabled
@Autonomous(name="Far Bucket", group="B")
public class farBucket extends OpMode {
    public int pathState;
    public farBucketAuto auto;
    ...
    public void pathUpdate() {
        switch (pathState) {
            case 0:
                ...
                break;
            case 1:
                if(!auto.follower.isBusy() && pathTimer.getElapsedTimeSeconds() > 15) {
//                    auto.startRetract();
...
//                    auto.startIntake();
                    auto.follower.followPath(auto.commonPreScore, true);
                    setPathState(3);
                }
                break;
            case 3:
                if(!auto.follower.isBusy() && auto.actionNotBusy()) {
//                    auto.startRetract();
                    auto.follower.followPath(auto.oppositeControl, true);
                    setPathState(4);
                }
                break;
            case 4:
                if(!auto.follower.isBusy() && auto.actionNotBusy()) {
//                    auto.startBucket();
                    auto.follower.followPath(auto.oppositeScore, true);
                    setPathState(5);
                }
                break;
            case 5:
